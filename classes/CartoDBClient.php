...
require_once Config::get('root').'/classes/Oauth.php';
...
//    $values = implode(',', array_values($data));
    ...
    $sql = "INSERT INTO $table ($keys) VALUES($qLine);";
  ...
  public function updateRow($table, $row_id, $data) {
	...
	$qLine = substr(trim($qLine), 0, -1);
    $sql = "UPDATE $table SET ($keys) = ($qLine) WHERE cartodb_id = $row_id;";
    /*$sql .= "SELECT $table.cartodb_id as id, $table.* FROM $table ";
    $sql .= "WHERE cartodb_id = currval('public." . $table . "_cartodb_id_seq');";
