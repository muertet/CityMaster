...
class MysqliDb
{
     ...
     * @return array Contains the returned rows from the query.
     */
    public function unSafeQuery($query, $numRows = null)
    {
		$this->_query = str_replace('&#39;',"'",$query);//apanyo
		//echo $this->_query;
    ...
    {
        $this->_query = filter_var($query, FILTER_SANITIZE_STRING);
		$this->_query=str_replace('&#39;',"'",$this->_query);//apanyo
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->reset();
   ...
   /**
    ...
    public function insert($tableName, $insertData)
    {
        ...
        $stmt->execute();
        $this->reset();
	//Util::p($this->_query,$this->_mysqli->error);//if fails enable this
    ...
    public function update($tableName, $tableData)
    {
        ...
        $stmt->execute();
//Util::p($this->_query,$this->_where,$this->_mysqli->error);//if fails enable this
    ...
    protected function _prepareQuery()
    {
        if (!$stmt = $this->_mysqli->prepare($this->_query)) {
			if($_SESSION['underControl'] || true){
				throw new Exception("Problem preparing query ($this->_query) " . $this->_mysqli->error, E_USER_ERROR);
			}else{
				throw new Exception('Mongo query server down');
			}
        }
        return $stmt;
    }
